...
--            COMMIT;
        ...
        EXCEPTION 
          WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE(CUR.CADENA||'error');
        end;    
    END LOOP;
    
    DBMS_OUTPUT.PUT_LINE(
        'Cadenas sin CT: '||V_SINCT_C||' ('||V_SINCT_S||' clientes) '||
        'Cadenas no encontradas: '||V_SINCO_C||' ('||V_SINCO_S||' clientes) '||
        'Cadenas Modificadas: '||V_CONCT_C||' ('||V_CONCT_S||' clientes) '
        );
END;
/
spool off;